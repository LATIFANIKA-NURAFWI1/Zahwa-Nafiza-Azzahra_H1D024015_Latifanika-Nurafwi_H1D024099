...
export default function AuthPage() {
  ...
  return (
    <div className="fade-in">
      <h1 className="text-3xl font-bold text-primary-dark">{mode==='login'? 'Masuk' : 'Daftar'}</h1>
      <div className="card p-6 mt-6 max-w-md">
        <form onSubmit={submit} className="grid gap-3">
          {mode==='register' && (
            <div>
              <label className="text-sm text-gray-600">Nama</label>
              <input className="w-full border rounded-lg px-3 py-2" value={form.name} onChange={e=>setForm({...form, name:e.target.value})} required={mode==='register'} />
            </div>
          )}
          <div>
